...
# Build & test
build:; forge build
test:; forge test --no-match-path "**.integration.t.sol" -vvv ${args} # --ffi # enable if you need the `ffi` cheat code on HEVM
test-integration:; forge test --match-path "**.integration.t.sol" --fork-url ${FORK_URL} -vvv ${args}
