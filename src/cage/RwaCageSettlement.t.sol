...
contract RwaCageSettlementTest is Test {
    ...
    // Asserts against gem and coin balance changes of the sender and the settlement
    // contract, before and after calling `redeem` with an arbitrary value.
    ...
    // Asserts against the coin balances of both the sender and the settlement
    // contract, before and after calling `deposit` with an arbitrary value.
    ...
    // Asserts that is not possible to redeem less than `minRedeemable`.
    // Asserts that is always possible redeem `minRedeemable`, given enough gems
    ...
    // Attempts to redeem a smaller amount first.
    // Redeem either the remaining balance or the maximum redeemable,
    // depending on how rounding errors played out.
    // Assets that the remaining redeemable amount is negligible.
