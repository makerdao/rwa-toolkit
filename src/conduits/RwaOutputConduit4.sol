...
contract RwaOutputConduit4 {
    ...
    modifier auth() {
        require(wards[msg.sender] == 1, "RwaOutputConduit4/not-authorized");
        _;
    }

    modifier onlyMate() {
        require(may[msg.sender] == 1, "RwaOutputConduit4/not-mate");
        _;
    }
    ...
    constructor(
        ...
        require(PsmAbstract(_psm).dai() == _dai, "RwaOutputConduit4/wrong-dai-for-psm");
        require(GemJoinAbstract(PsmAbstract(_psm).gemJoin()).gem() == _gem, "RwaOutputConduit4/wrong-gem-for-psm");
    ...
    function file(bytes32 what, address data) external auth {
        if (what == "quitTo") {
            ...
            require(PsmAbstract(data).dai() == address(dai), "RwaOutputConduit4/wrong-dai-for-psm");
            require(
                GemJoinAbstract(PsmAbstract(data).gemJoin()).gem() == address(gem),
                "RwaOutputConduit4/wrong-gem-for-psm"
            );
            ...
            revert("RwaOutputConduit4/unrecognised-param");
        }
    ...
    function pick(address who) external {
        require(can[msg.sender] == 1, "RwaOutputConduit4/not-operator");
        require(bud[who] == 1 || who == address(0), "RwaOutputConduit4/not-bud");
        to = who;
        emit Pick(who);
    }
    ...
    function _doPush(uint256 wad) internal {
        require(to != address(0), "RwaOutputConduit4/to-not-picked");
        ...
        require(gemAmt > 0, "RwaOutputConduit4/insufficient-swap-gem-amount");
    ...
    function _doQuit(uint256 wad) internal {
        require(quitTo != address(0), "RwaOutputConduit4/invalid-quit-to-address");
